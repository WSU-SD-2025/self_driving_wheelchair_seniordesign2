...
void WheelchairController::normalizedToVoltage(float uy, float ux, float& y_out_v, float& x_out_v){
    uy = clampValue(uy, -1.0f, 1.0f);
    ux = clampValue(ux, -1.0f, 1.0f);

    float uy_eff = applyDeadband(uy, y_deadband);
    float ux_eff = applyDeadband(ux, x_deadband);

    y_out_v = y_neutral + (uy_eff * y_span);
    x_out_v = x_neutral + (ux_eff * x_span);
