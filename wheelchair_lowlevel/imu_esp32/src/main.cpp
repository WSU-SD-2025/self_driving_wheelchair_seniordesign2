...
// Print period
static const uint32_t PRINT_PERIOD_MS = 50;   // 20 Hz
...
static void printImuState() {
  ...
  // Quaternion (ROS order: x y z w)
  float qw = quat_a_raw / QUAT_SCALE;
  float qx = quat_b_raw / QUAT_SCALE;
  float qy = quat_c_raw / QUAT_SCALE;
  float qz = quat_d_raw / QUAT_SCALE;

  // Gyro (deg/s -> rad/s)
  float wx = gyro_x_dps * PI / 180.0f;
  float wy = gyro_y_dps * PI / 180.0f;
  float wz = gyro_z_dps * PI / 180.0f;

  // Accel already m/s^2
  float ax = accel_x_mps2;
  float ay = accel_y_mps2;
...
static bool try_parse_one_packet(Um7Packet& out, size_t& consumed) {
  ...
  size_t total_len = 3 + 1 + 1 + data_len + 2; // snp + packet + addr + data + checksum
  ...
  uint16_t recv =
      ((uint16_t)rx_buffer[total_len - 2] << 8) |
      rx_buffer[total_len - 1];
  ...
  if (data_len > 0) {
    memcpy(out.data, rx_buffer + 5, data_len);
  }
...
void loop() {
  ...
  // Print latest full IMU state at fixed rate
  uint32_t now = millis();
